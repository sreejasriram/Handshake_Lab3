...
class Applications extends Component {
    ...
    viewapplicants=async()=>{
        const { data } = await this.props.client.query({
            query: listAppliedJobs,
            variables: { studentId: sessionStorage.getItem("studentId") },
            fetchPolicy: 'no-cache'
        })
        console.log(data)
            if (data.listAppliedJobs) {
            this.setState({
                dataRetrieved: true,
                jobData: data.listAppliedJobs
            });
                    ...
                    return (
                        <div key={data._id}>
                           <Card>
                            ...
                            {data.applications?(<div><EventAvailableOutlinedIcon fontSize="small" style={{ color: "#1569E0" }}></EventAvailableOutlinedIcon> {data.applications[0].appliedDate.substring(0,10)}</div>):<div></div>} 
                            </div> <div class="col-md-2" >
